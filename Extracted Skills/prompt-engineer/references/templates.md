...
Full template library for prompt building. Load the relevant template when the user's task type matches. Do not load all templates at once.
...
*Use for logic-heavy tasks, math, debugging, and multi-factor analysis. Only use CoT for standard reasoning models (Claude, GPT-4o, Gemini). Do NOT add CoT instructions to o3, o4-mini, DeepSeek-R1, or Qwen3 thinking mode.*
...
*Use when the output format is easier to show than describe. Examples outperform written instructions for format-sensitive tasks.*
...
Rules: 2-5 examples is the sweet spot. Include edge cases. Use XML tags; Claude parses them reliably. If re-prompting for the same formatting correction twice, switch to few-shot.
...
- **Midjourney**: Comma-separated descriptors. Add `--ar`, `--style`, `--v 6` at the end.
...
- Stable Diffusion: use img2img mode, not txt2img. Set denoising strength 0.3-0.6
...
*Use for ComfyUI node-based workflows. Always output Positive and Negative prompts as separate blocks.*
...
*Use when the user pastes an existing prompt and wants to break it down, adapt it for a different tool, simplify it, or understand its structure.*

Detect which task is needed: Break down, Adapt, Simplify, or Split.
